...
/*:
 * @plugindesc (v3.6) Permite customizar o layout de batalha.
 * @author Moghunter
 *
 * @param Hud X-Axis
 ...
 * da Hud. (false - Define a posicao fixa.)
 * @default true
 *
 ...
 * (3.5) - Correcao na apresentacao de nomes longos.
 * (3.4) - Adicao do modo de apresentar as condicoes em linha.
 * (3.3) - Correcao do crash ao desativar as condicoes (states).
 * (3.2) - Adicao da animacao de Slide.
 *       - Adicao de novos efeitos no sprite de turno.
 *       - Adicao do parametro de alinhar o nome.
 *       - Compatibilidade com Victor Active Time Battle.
 * (3.1) - Correcao do crash quando a face e desativada.
 * (3.0) - Correcao da funcao da face animada.
 * (2.9) - Modificacao da posicao do layout (Overlay) acima dos medidores .
 * (2.8) - Adicao do comando para ocultar a hud.
 * (2.7) - Correcao de alguns plugins parameters.
 * (2.6) - Correcao do parametro do Screen Layout Y nao funcionar.
 * (2.5) - Correcao de alguns plugins parameters nao funcionarem.
 * (2.4) - Correcao de nao apresentar os icones de Buff e Debuff.
 * (2.3) - Melhoria na codificacao.
 * (2.2) - Adicao da opcao do alinhamento dos numeros.
 *       - Adicao do layout Overlay.
 * (2.1) - Compatibilidade com MOG Battle Cursor no modo Front View.
 * (2.0) - Faces como actors no modo Frontview.
 *       - Opcao de definir a quantidade battlers na batalha.
 * (1.5) - Correcao no setup do angulo dos medidores.
 * (1.4) - Correcao na prioridade do layout Screen.
 * (1.3) - Correcao na posicao do plugin do template.
 * (1.2) - Adicao do modo hud na vertical.
 *       - Opcao de definir o espaco entre as huds.
 *       - Opcao de definir uma posicao absoluta para a janela de comando.
 *       - Opcao de definir posicoes especificas das huds.
 *       - Funcao de faces animadas por frames.
 *       - Opcao da animacao em gradiente do medidor.
 * (1.1) - Opcao de poder definir a prioridade da face.
 *               (0 - Abaixo do layout. 1 - Acima do layout.)
 *       - Opcao de definir a quantidade de colunas da janela.
 *       - Correcao da posicao das condicoes atraves do setup.
 *
 */
/*~struct~FaceData:
 *
 ...
 *
 * @param Face Image
 * @desc Actor Facd Image(img/battlehud/)
 * @type file
 * @require 1
 * @dir img/battlehud
 *
...
//=============================================================================
	...
	Moghunter.parameters = paramParse(PluginManager.parameters('MOG_BattleHud'));
	...
	Moghunter.bhud_tp_meter_pos_x = Number(Moghunter.parameters['TP Meter X-Axis'] || 47);
	...
	// Actor Face Data Duplicate Check
	var duplicate_list = [];
	Moghunter.bhud_face_list.forEach(function(data) {
		if(data['Actor ID'] in duplicate_list) {
			duplicate_list[data['Actor ID']].push(data);
		} else {
			duplicate_list[data['Actor ID']] = [data];
		}
	}, this);

	var is_duplicate = false;
	Object.keys(duplicate_list).forEach(function(key) {
		if(duplicate_list[key].length > 1) {
			is_duplicate = true;
			console.log("duplicate actor data:%o", duplicate_list[key]);
		}
	}, this);

	if(is_duplicate) {
		alert("[MOG_BattleHud] Actor Data is Duplicate!! Plese check console(F8)");
	}
...
// ** create Upper Layer
//==============================
var _mog_bhud_sprtbase_createUpperLayer = Spriteset_Base.prototype.createUpperLayer;
Spriteset_Base.prototype.createUpperLayer = function() {
	_mog_bhud_sprtbase_createUpperLayer.call(this);
...
// ** remove Battle Hud
//==============================
Spriteset_Base.prototype.removeBattleHud = function() {
	if (!this._hudField) {this.createHudField()};
	if (String(Moghunter.bhud_screen_layout) === "true") {this.createBattleHudScreenLayout();};
	$gameTemp.refresh_Bhud = false;
	$gameTemp._battleEnd = false;
...
Spriteset_Base.prototype.removeBattleHud = function() {
	if (this._screen_layout) {
		if (this._chronoField) {
	        this._chronoField.removeChild(this._screen_layout);
		} else {
	...
	for (var i = 0; i < this._battle_hud.length; i++) {
		if (this._chronoField) {
		   this._chronoField.removeChild(this._battle_hud[i]);
		} else {
...
//==============================
Spriteset_Base.prototype.createBattleHudScreenLayout = function() {
	...
	if (this._chronoField) {
        this._chronoField.addChild(this._screen_layout);
	} else {
...
//==============================
Spriteset_Base.prototype.updateBattleHudVisible = function() {
...
//==============================
Spriteset_Base.prototype.isBattleHudVisible = function() {
...
// ** Refresh Battle Hud
//==============================
Spriteset_Base.prototype.refreshBattleHud = function() {
...
// ** createWindowLayer
//==============================
var _alias_mog_sprtBattle_bhud_initialize = Spriteset_Battle.prototype.initialize
Spriteset_Battle.prototype.initialize = function() {
	_alias_mog_sprtBattle_bhud_initialize.call(this);
...
Battle_Hud.prototype.create_face = function() {
	...
	if (!this._battler) {return};
	face_data = Moghunter.bhud_face_list.filter(function(data) {
		return data["Actor ID"] == this._battler._actorId;
	}, this);
	if (!face_data[0]) {return};
	if (!face_data[0]["Face Image"]) {return};
	// this._face = new Sprite(ImageManager.loadBHud("Face_" + this._battler._actorId));
	this._face = new Sprite(ImageManager.loadBHud(face_data[0]["Face Image"]));
